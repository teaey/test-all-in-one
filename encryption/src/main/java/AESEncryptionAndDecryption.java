 ...
 * @author xiaofei.wxf
 */
public class AESEncryptionAndDecryption implements EncryptionAndDecryption {
    ...
    public AESEncryptionAndDecryption(byte[] key) {
    ...
    public static byte[] newKey() throws NoSuchAlgorithmException {
        KeyGenerator keyGen = KeyGenerator.getInstance(ALGORITHM);
        keyGen.init(256);
        SecretKey secretKey = keyGen.generateKey();
        return secretKey.getEncoded();
