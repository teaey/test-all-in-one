        ...
        public void run() {
            ...
            try {
                while ((readLength = fis.read(buffer)) > -1) {
                    int tohandle =
                        readLength + handleNum > totalNum ? (totalNum - handleNum) : readLength;
                    for (int i = 0; i < tohandle; i++) {
                        byte readed = buffer[i];
                        if (readed == empty || readed == dian || readed == enter
                            || readed == yinhang || readed == douhao || readed == danyinhao
                            || readed == langhao || readed == wenhao || readed == tanhao
                            || readed == jianhao || readed == fenhao) {
                            //if (split(readed)) {
                            if (sb.length() > 0) {
                            ...
                            sb.append(
                                (char) ((readed >= 65 && readed <= 90) ? (readed + 32) : readed));
                        }
                    }
                }
                fis.close();
